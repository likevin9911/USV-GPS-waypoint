...
{
  // Set up publishers.
  pub_course_      = nh_.advertise<usv_msg::Course>("/course_cmd", 1);
...
void WaypointFollower::waypointCb(const usv_msg::WaypointRoute::ConstPtr& msg)
{
...
void WaypointFollower::evaluateWaypoint()
{
  ...
  {
    case usv_msg::Waypoint::NAV_WAYPOINT:
      //
      WaypointFollower::setNextWaypoint(); // Set next waypoint
      break;

    case usv_msg::Waypoint::NAV_STATION:
      //
...
void WaypointFollower::assignCourse(usv_msg::Course& msg, bool station)
{
...
{
  usv_msg::Course course_cmd; // Course command for course controller
